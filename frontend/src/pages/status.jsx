...
const IconMA = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <rect x="2" y="5" width="20" height="15" rx="2"/><path d="M2 10h20"/>
  </svg>
);
  ...
  return (
    <div
      ...
      style={{
        ...
        border: checked ? "1px solid #16a34a" : "1px solid rgba(255,255,255,0.15)"
      }}
    >
      <div style={{
...
// ── Phone Preview ─────────────────────────────────────────────────────────────
function PhonePreview({ message, customerName = "Growai Client" }) {
  ...
  return (
    <div style={{
      width: "220px", background: "#1a1a2e", borderRadius: "32px", overflow: "hidden",
      border: "6px solid #0d0d1a", boxShadow: "0 24px 60px rgba(0,0,0,0.6)",
      flexShrink: 0
    }}>
      {/* Status bar */}
      ...
      <div style={{
        ...
        padding: "12px 8px", minHeight: "200px"
      }}>
        <div style={{ textAlign: "center", fontSize: "0.6rem", color: "rgba(255,255,255,0.35)", background: "rgba(255,255,255,0.05)", borderRadius: "8px", padding: "3px 8px", display: "inline-block", margin: "0 auto 10px", display: "block" }}>
          Today
        </div>
...
// ── Language Card ─────────────────────────────────────────────────────────────
function LangCard({ lang, message, onToggle, active, selected, onSelect }) {
  ...
  return (
    <div
      onClick={onSelect}
      style={{
        background: selected ? "rgba(34,197,94,0.08)" : "var(--bg-app, #111)",
        border: selected ? "1px solid rgba(34,197,94,0.4)" : "1px solid var(--border-color, #2a2a2e)",
    ...
    >
      ...
      <div style={{
        ...
        color: "#fff", fontSize: "0.5rem", fontWeight: "800", padding: "3px 28px",
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: "12px" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: "0.71rem", color: "var(--text-muted,#888)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {preview || <em>No template set</em>}
          </div>
        </div>
      </div>
    </div>
...
export default function Status() {
  ...
  // Bootstrap statuses from backend or fallback to defaults
  useEffect(() => {
    const load = async () => {
      try {
        const { data } = await api.get("/order-statuses");
        const list = data.length > 0 ? data : Object.entries(STATUS_META).map(([slug, meta], i) => ({
          id: i + 1, slug, name: meta.label
        }));
        setStatuses(list);
        setSelectedStatus(list[0]);

        // Seed templates & autoSend state
        const tpls = {};
        const as = {};
        list.forEach(s => {
          tpls[s.slug] = s.templates || DEFAULT_TEMPLATES[s.slug] || {};
          as[s.slug] = s.auto_send || false;
        });
        setTemplates(tpls);
        setAutoSend(as);
      } catch {
  ...
  const handleTemplateChange = (val) => {
    ...
    setTemplates(prev => ({
      ...prev,
      [selectedStatus.slug]: { ...(prev[selectedStatus.slug] || {}), [selectedLang]: val }
    }));
  };
  ...
  const handleToggleAutoSend = async () => {
    ...
    try {
      await api.post(`/order-statuses/${selectedStatus.id}/auto-send`, { auto_send: next });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await api.put(`/order-statuses/${selectedStatus.id}`, {
    ...
    } catch { /* show anyway */ setSavedMsg(true); setTimeout(() => setSavedMsg(false), 2500); }
    ...
    <div style={{ color: "var(--text-main,#fff)", minHeight: "100%", paddingBottom: "40px" }}>
      ...
      {savedMsg && (
        <div style={{
          ...
          boxShadow: "0 4px 16px rgba(34,197,94,0.4)", display: "flex", gap: "8px", alignItems: "center"
        }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px" }}>
        <div>
          <h2 style={{ display: "flex", alignItems: "center", gap: "10px", fontSize: "1.4rem", fontWeight: "800", margin: 0 }}>
            <div style={{ width: "32px", height: "32px", borderRadius: "8px", background: "rgba(114,57,234,0.15)", display: "flex", alignItems: "center", justifyContent: "center", color: "#7239ea" }}>
              <IconShield/>
            </div>
            Status
          </h2>
          <p style={{ color: "var(--text-muted,#888)", fontSize: "0.82rem", marginTop: "4px" }}>
            Configure confirmation and delivery statuses with WhatsApp templates
          </p>
        </div>
        ...
        <button
          ...
          style={{
            ...
            padding: "10px 22px", borderRadius: "10px", fontSize: "0.85rem", fontWeight: "700",
            background: saving ? "rgba(114,57,234,0.5)" : "#7239ea",
            color: "#fff", border: "none", cursor: saving ? "not-allowed" : "pointer",
            boxShadow: "0 4px 16px rgba(114,57,234,0.3)", transition: "all 0.2s"
          }}
        >
          <IconSave/>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "20px" }}>
        {[
          ...
          <button
            ...
            style={{
              display: "flex", alignItems: "center", gap: "8px",
              padding: "9px 18px", borderRadius: "10px", fontSize: "0.82rem", fontWeight: "700",
              background: activeTab === tab.key ? "var(--bg-card,#18181b)" : "transparent",
              color: activeTab === tab.key ? "var(--text-main,#fff)" : "var(--text-muted,#888)",
              border: activeTab === tab.key ? "1px solid var(--border-color,#2a2a2e)" : "1px solid transparent",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "220px 1fr 240px", gap: "16px", alignItems: "start" }}>
        ...
        {/* ── LEFT: Status list ── */}
        <div style={{
          background: "var(--bg-card,#18181b)", border: "1px solid var(--border-color,#2a2a2e)",
          ...
          <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border-color,#2a2a2e)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: "0.65rem", fontWeight: "800", letterSpacing: "0.08em", color: "var(--text-muted,#888)", textTransform: "uppercase" }}>
              Confirmation Statuses
            </span>
            <div style={{ width: "20px", height: "20px", borderRadius: "50%", background: "rgba(255,255,255,0.06)", display:"flex",alignItems:"center",justifyContent:"center", cursor:"pointer", color:"var(--text-muted)" }}>
              <IconInfo/>
            </div>
          </div>
          ...
          <div style={{ padding: "6px" }}>
            {statuses.map(s => {
              ...
              return (
                <div
                  key={s.id}
                  onClick={() => { setSelectedStatus(s); setSelectedLang("FR"); }}
                  style={{
                    display: "flex", alignItems: "center", gap: "10px",
                    padding: "10px 12px", borderRadius: "9px", cursor: "pointer",
                    background: isSelected ? "rgba(114,57,234,0.15)" : "transparent",
                    border: isSelected ? "1px solid rgba(114,57,234,0.3)" : "1px solid transparent",
                ...
                >
                  ...
                  <span style={{ fontSize: "0.82rem", fontWeight: isSelected ? "700" : "500", color: isSelected ? "#c4a7ff" : "var(--text-main,#fff)" }}>
                    {s.name || meta.label}
                  </span>
                </div>
              );
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          {selectedStatus && (
            <div style={{
              background: "var(--bg-card,#18181b)", border: "1px solid var(--border-color,#2a2a2e)",
              borderRadius: "14px", padding: "14px 18px", display: "flex", alignItems: "center", gap: "14px"
            }}>
              ...
              <input
                value={selectedStatus.name || statusMeta.label || ""}
                onChange={e => setStatuses(prev => prev.map(s => s.id === selectedStatus.id ? { ...s, name: e.target.value } : s))}
                style={{
                  flex: 1, background: "transparent", border: "none", outline: "none",
                  fontSize: "1.1rem", fontWeight: "700", color: "var(--text-main,#fff)"
                }}
              />
              {/* Color dot selector */}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: "0.7rem", color: "var(--text-muted,#888)" }}>
                    Auto-sent when order moves to this status
                  </div>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                <span style={{ fontSize: "0.72rem", fontWeight: "600", color: currentAutoSend ? "#22c55e" : "var(--text-muted,#888)" }}>
                  Auto-send {currentAutoSend ? "ON" : "OFF"}
                </span>
            ...
            <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "10px" }}>
              {LANGS.map(lang => (
                <LangCard
                  key={lang}
                  lang={lang}
                  message={templates[selectedStatus?.slug]?.[lang] || ""}
          ...
          {selectedStatus && (
            <div style={{
                ...
                <div style={{ fontSize: "0.7rem", color: "var(--text-muted,#888)", display: "flex", gap: "8px" }}>
                  {["{{customer_name}}", "{{order_id}}", "{{shop_name}}", "{{total}}"].map(v => (
                    <code key={v} style={{ background: "rgba(114,57,234,0.15)", color: "#c4a7ff", borderRadius: "4px", padding: "2px 6px", fontSize: "0.65rem", cursor: "pointer" }}
                      onClick={() => handleTemplateChange(currentTemplate + v)}
                    >{v}</code>
                  ))}
                </div>
              </div>
              <textarea
                ...
                style={{
                  width: "100%", minHeight: "140px", background: "transparent", border: "none", outline: "none",
                  color: "var(--text-main,#fff)", fontSize: "0.85rem", lineHeight: "1.6",
                  padding: "16px 18px", resize: "vertical", boxSizing: "border-box",
                  fontFamily: "inherit"
                }}
              />
