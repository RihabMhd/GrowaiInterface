    ...
    <header className="top-navbar">
      <div className="navbar-left">
        <div className="sidebar-logo" style={{ display: "flex", alignItems: "center", paddingLeft: "10px" }}>
          <span style={{ fontSize: "1.35rem", fontWeight: 800, letterSpacing: "0.5px", color: "var(--text-navbar)" }}>Growai</span>
        </div>
      </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: '0.85rem',
                      fontWeight: 600,
                      color: isActive ? '#fff' : 'var(--text-popup-strong)',
          ...
          <div className="nav-item-profile-container" style={{ position: 'relative' }}>
            ...
            >
              {user.avatar_url ? (
                ...
                <div style={{
                  ...
                  backgroundColor: "var(--danger)", color: "white", display: "flex",
            ...
            {isProfileDropdownOpen && (
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px", borderBottom: "1px solid var(--border-popup)", marginBottom: "6px" }}>
                  {user.avatar_url ? (
                    <img src={user.avatar_url} alt={user.name} style={{ width: "32px", height: "32px", borderRadius: "50%", objectFit: "cover" }} />
                  ) : (
<div style={{ width: "32px", height: "32px", borderRadius: "50%", backgroundColor: "var(--danger)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "bold" }}>
                  {user.name ? user.name.charAt(0).toUpperCase() : "R"}
                    </div>
                  )}
                  <div style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
                    <strong style={{ color: "var(--text-popup-strong)", fontSize: "0.85rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.name}</strong>
                    <span style={{ color: "var(--text-popup-span)", fontSize: "0.75rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.email}</span>
                  </div>
                </div>

                <button
                  onClick={() => { navigate("/settings"); setIsProfileDropdownOpen(false); }}
                  style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 12px", width: "100%", background: "none", color: "var(--text-popup-strong)", fontSize: "0.8rem", fontWeight: "600", borderRadius: "6px", cursor: "pointer", textAlign: "left" }}
                ...
                <hr style={{ border: "0", borderTop: "1px solid var(--border-popup)", margin: "6px 4px" }} />

                <button
                  onClick={() => { logout(); setIsProfileDropdownOpen(false); }}
                  style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 12px", width: "100%", background: "none", color: "var(--danger)", fontSize: "0.8rem", fontWeight: "700", borderRadius: "6px", cursor: "pointer", textAlign: "left" }}
                  className="dropdown-item-hover-logout"
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ color: "var(--danger)" }}><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg>
                  Déconnexion
                </button>
              </div>
            )}
            <style>{`
              .navbar-profile-hover:hover { background-color: var(--bg-popup-hover); }
              .dropdown-item-hover:hover { background-color: var(--bg-popup-hover); }
